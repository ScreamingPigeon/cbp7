...
#define TA2C_BASE_U(UW)                                                        \
    TATableConfig<28, 2048, 12, 4, 1000, 2,                                   \
                  ...
                  ta::UniformTag<12>,                                          \
                  ta::StepSize<4096, 2048, 24>>,                               \
    8, 6, 3, true, 1, ta::DefaultSecTagHash, 1, 2, UW, 4096, false, 6,        \
    4, 256, 2, 1024, true, HistUpdate::PATH, TADefaultAllocConfig,             \
...
using TA2C_B = TageAhead<TA2C_BASE,
    false, DecayMiss::TAG, DecayOp::DECREMENT,        // DECAY_ENABLE=false
...
using TA2C_C = TageAhead<TA2C_BASE,
    true, DecayMiss::TAG_OR_SEC, DecayOp::DECREMENT,
    ta::uniform_array<u64, 28>(8), ta::DefaultDecayThresh,
    false>;

// D: TAG only, LW=8 (less aggressive trigger)
using TA2C_D = TageAhead<TA2C_BASE,
    true, DecayMiss::TAG, DecayOp::DECREMENT,
    ta::uniform_array<u64, 28>(8), ta::DefaultDecayThresh,
    false>;

// E: TAG_AND_SEC, LW=8 (least aggressive trigger)
using TA2C_E = TageAhead<TA2C_BASE,
    true, DecayMiss::TAG_AND_SEC, DecayOp::DECREMENT,
    ta::uniform_array<u64, 28>(8), ta::DefaultDecayThresh,
    false>;

// F: TAG_OR_SEC, LW=12 (lower fire probability)
using TA2C_F = TageAhead<TA2C_BASE,
    true, DecayMiss::TAG_OR_SEC, DecayOp::DECREMENT,
    ta::uniform_array<u64, 28>(12), ta::DefaultDecayThresh,
    false>;

// G: TAG_OR_SEC, LW=6 (higher fire probability)
using TA2C_G = TageAhead<TA2C_BASE,
    true, DecayMiss::TAG_OR_SEC, DecayOp::DECREMENT,
    ta::uniform_array<u64, 28>(6), ta::DefaultDecayThresh,
    false>;

// H: TAG_OR_SEC, per-table LW: 12 for T0-13, 6 for T14-27
using TA2C_H = TageAhead<TA2C_BASE,
    true, DecayMiss::TAG_OR_SEC, DecayOp::DECREMENT,

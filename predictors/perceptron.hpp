...
template <u64 HIST_LEN = 32, u64 LOGN = 2, u64 LOG_ENTRIES = 8,
          u64 LOGBBMAX = 6>
struct yuwei_perceptron : predictor {
  ...
  static constexpr u64 WEIGHT_BITS =
      static_cast<u64>(std::bit_width(THRESHOLD));
  static constexpr u64 SUM_BITS = WEIGHT_BITS + std::bit_width(HIST_LEN + 1);
  static constexpr u64 BBMAX =
      1 << LOGBBMAX; // maximum basic block size (in instructions)
  ...
  void block_predict(val<64> inst_pc) {
    ...
    arr<val<SUM_BITS, i64>, N> sums = [&](u64 i) -> val<SUM_BITS, i64> {
      ...
      arr<val<WEIGHT_BITS, i64>, HIST_LEN + 1> flipped_weights =
          [&](u64 j) -> val<WEIGHT_BITS, i64> {
        if (j == 0) {
    ...
    // training-time).
    for (u64 i = 0; i < N; i++) {
      below_threshold[i] = (sums[i] < static_cast<i64>(THRESHOLD)) &
                           (sums[i] > -static_cast<i64>(THRESHOLD));
    }
    ...
    pred_taken.fanout(
        hard<BBMAX * 4>{}); // FIXME reduce to 2xBBMAX if/when making perceptron
                            // only second-level predictor
  };
  ...
  void update_cycle([[maybe_unused]] instruction_info &block_end_info) {
    ...
    if (num_condbr == 0) {
      ...
      execute_if(block_end_info.is_taken, [&]() {
        global_history = global_history.shift_left(
            val<2>{block_end_info.next_pc.fo1() >> 2});
      });
      bb_inst = 0;
      return;
    }
    ...
    arr<val<N>, N> taken_onehot = [&](u64 i) {
      return banks_with_branches_onehot[i] &
             branch_exec_taken[i].replicate(hard<N>{}).concat();
    };
    ...
    arr<val<N>, N> mispredicted_onehot = [&](u64 i) {
      ...
      // this prediction block
      val<N> valid_mask =
          val<1>{i == (num_condbr - 1)}.replicate(hard<N>{}).concat();
      val<N> mispredicted_mask = mispredict.replicate(hard<N>{}).concat();
      return valid_mask.fo1() & mispredicted_mask.fo1() &
             (X ^ i).decode().concat();
    };
    val<N> mispredicted_mask = mispredicted_onehot.fo1().fold_or();

    // Which banks are performing an update?
    val<N> update_mask = mispredicted_mask.fo1() |
                         (below_threshold.fo1().concat() & branches_mask.fo1());
      ...
      execute_if(update[i].fo1(), [&]() {
        arr<val<WEIGHT_BITS, i64>, HIST_LEN + 1> updated_vector = [&](u64 j) {
          if (j == 0)
            ...
            return update_ctr(site[i].branch_weights[j],
                              taken[i] ^ global_history[j - 1]);
        };
    ...
    execute_if(block_end_info.is_taken, [&] {
      global_history =
          global_history.shift_left(val<2>{block_end_info.next_pc.fo1() >> 2});
    });

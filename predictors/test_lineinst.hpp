...
// MODE: 0 = normal reuse_prediction(~line_end())
//       1 = no reuse_prediction at all
//       2 = compute line_end() but discard, pass hard<1> to reuse_prediction
//       3 = pass hard<1> to reuse_prediction (no line_end computation)
// REUSE_FO: fanout budget for regs read in reuse_predict2 (pred1_tage, pred2_tage, altsel_bits)
template <u64 LINEINST_V, int MODE = 0, u64 REUSE_FO = 2>
struct test_lineinst : predictor {
    ...
    static constexpr u64 LOG_LINEINST = std::bit_width(LINEINST - 1);
    static constexpr u64 META_BITS = 4;
    static constexpr u64 META_PIPE = 2;

    template <u64 TABLE_SIZE, u64 TAG_WIDTH>
    struct MiniTable {
        static constexpr u64 IDX = std::bit_width(TABLE_SIZE - 1);
    ...
    val<1> predict1(val<64> inst_pc) override {
        ...
        if constexpr (MODE == 0) {
            ...
            // compute line_end but sink it; pass hard<1> to reuse_prediction
            val<1> le = line_end();
            le.fanout(hard<2>{});
    ...
    val<1> predict2(val<64> inst_pc) override {
        ...
        pred1_tage.fanout(hard<REUSE_FO>{});

        for (u64 r = 0; r < N; r++) {
            match2_reg[r] = (match_reg[r] ^ match1_reg[r]).one_hot();
        }
        match2_reg.fanout(hard<REUSE_FO>{});
        for (u64 r = 0; r < N; r++) {
            pred2_tage[r] = (match2_reg[r] & preds[r]) != hard<0>{};
        }
        pred2_tage.fanout(hard<REUSE_FO>{});
        ...
        // Direct-index return — block_entry NOT on this path

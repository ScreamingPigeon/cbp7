...
#include "../cbp.hpp"
#include "../harcom.hpp"
#include "custom/custom_common.hpp"

using namespace hcm;

// ============================================================================
// RABT — Run-Ahead Block TAGE with independent resolution chains
//
// Each TAGE entry stores a 1-bit prediction for one branch.
// The 11-bit tag is split into 8-bit htag + 3-bit group_id.
// 7 independent resolution chains run in parallel.
//
// Table sizes:
//   T0-T4:  1024 entries (IDX=10, HYST=512)
//   T5-T14: 2048 entries (IDX=11, HYST=1024)
// ============================================================================

struct newRABT : predictor {
  ...
  // ======== Independent resolution constants ========
  ...
  static constexpr std::array<u64, NUM_GROUPS> GROUP_TO_META_BANK = {0, 0, 0, 0,
                                                                     1, 1, 1};
  static constexpr u64 PRED_BITS = 1; // 1-bit prediction per entry
  static constexpr u64 FB_PRED_BITS =
      N * CTR_WIDTH; // = 7, fallback stays N-wide
  static constexpr u64 HTAG_WIDTH =
      TAG_WIDTH - GROUP_BITS; // = 8 (max, for pipeline regs)
  ...
  static constexpr std::array<u64, NT> PER_TABLE_HTAG = {8, 8, 7, 7, 7, 6, 6, 6,
                                                         5, 5, 5, 4, 4, 4, 4};
  ...
  static constexpr std::array<u64, NT> HYST_SIZE = {
      512,  512,  512,  512,  512,  1024, 1024, 1024,
  ...
  // ============================================================
  // Per-table zone layout (independent-resolution-optimized):
  ...
  hcm::zone zone0;
  ta_rwram<PRED_BITS, 1024, 4, 1> pred_ram0{"t0_pred"};
  ta_rwram<HYST_WIDTH, 512, 4, 1> hyst_ram0{"t0_hyst"};
  ta_rwram<U_WIDTH, 1024, 4, 1> u_ram0{"t0_u"};
  ...
  hcm::zone zone1;
  ta_rwram<PRED_BITS, 1024, 4, 1> pred_ram1{"t1_pred"};
  ta_rwram<HYST_WIDTH, 512, 4, 1> hyst_ram1{"t1_hyst"};
  ta_rwram<U_WIDTH, 1024, 4, 1> u_ram1{"t1_u"};
  ...
  hcm::zone zone2;
  ta_rwram<PRED_BITS, 1024, 4, 1> pred_ram2{"t2_pred"};
  ta_rwram<HYST_WIDTH, 512, 4, 1> hyst_ram2{"t2_hyst"};
  ta_rwram<U_WIDTH, 1024, 4, 1> u_ram2{"t2_u"};
  ...
  hcm::zone zone3;
  ta_rwram<PRED_BITS, 1024, 4, 1> pred_ram3{"t3_pred"};
  ta_rwram<HYST_WIDTH, 512, 4, 1> hyst_ram3{"t3_hyst"};
  ta_rwram<U_WIDTH, 1024, 4, 1> u_ram3{"t3_u"};
  ...
  hcm::zone zone4;
  ta_rwram<PRED_BITS, 1024, 4, 1> pred_ram4{"t4_pred"};
  ta_rwram<HYST_WIDTH, 512, 4, 1> hyst_ram4{"t4_hyst"};
  ta_rwram<U_WIDTH, 1024, 4, 1> u_ram4{"t4_u"};
  ...
  hcm::zone zone14;
  ta_rwram<PRED_BITS, 2048, 4, 1> pred_ram14{"t14_pred"};
  ta_rwram<HYST_WIDTH, 1024, 4, 1> hyst_ram14{"t14_hyst"};
  ta_rwram<U_WIDTH, 2048, 4, 1> u_ram14{"t14_u"};
  ...
  hcm::zone zone5;
  ta_rwram<PRED_BITS, 2048, 4, 1> pred_ram5{"t5_pred"};
  ta_rwram<HYST_WIDTH, 1024, 4, 1> hyst_ram5{"t5_hyst"};
  ta_rwram<U_WIDTH, 2048, 4, 1> u_ram5{"t5_u"};
  ...
  hcm::zone zone6;
  ta_rwram<PRED_BITS, 2048, 4, 1> pred_ram6{"t6_pred"};
  ta_rwram<HYST_WIDTH, 1024, 4, 1> hyst_ram6{"t6_hyst"};
  ta_rwram<U_WIDTH, 2048, 4, 1> u_ram6{"t6_u"};
  ...
  hcm::zone zone7;
  ta_rwram<PRED_BITS, 2048, 4, 1> pred_ram7{"t7_pred"};
  ta_rwram<HYST_WIDTH, 1024, 4, 1> hyst_ram7{"t7_hyst"};
  ta_rwram<U_WIDTH, 2048, 4, 1> u_ram7{"t7_u"};
  ...
  hcm::zone zone8;
  ta_rwram<PRED_BITS, 2048, 4, 1> pred_ram8{"t8_pred"};
  ta_rwram<HYST_WIDTH, 1024, 4, 1> hyst_ram8{"t8_hyst"};
  ta_rwram<U_WIDTH, 2048, 4, 1> u_ram8{"t8_u"};
  ...
  hcm::zone zone9;
  ta_rwram<PRED_BITS, 2048, 4, 1> pred_ram9{"t9_pred"};
  ta_rwram<HYST_WIDTH, 1024, 4, 1> hyst_ram9{"t9_hyst"};
  ta_rwram<U_WIDTH, 2048, 4, 1> u_ram9{"t9_u"};
  ...
  hcm::zone zone10;
  ta_rwram<PRED_BITS, 2048, 4, 1> pred_ram10{"t10_pred"};
  ta_rwram<HYST_WIDTH, 1024, 4, 1> hyst_ram10{"t10_hyst"};
  ta_rwram<U_WIDTH, 2048, 4, 1> u_ram10{"t10_u"};
  ...
  hcm::zone zone11;
  ta_rwram<PRED_BITS, 2048, 4, 1> pred_ram11{"t11_pred"};
  ta_rwram<HYST_WIDTH, 1024, 4, 1> hyst_ram11{"t11_hyst"};
  ta_rwram<U_WIDTH, 2048, 4, 1> u_ram11{"t11_u"};
  ...
  hcm::zone zone12;
  ta_rwram<PRED_BITS, 2048, 4, 1> pred_ram12{"t12_pred"};
  ta_rwram<HYST_WIDTH, 1024, 4, 1> hyst_ram12{"t12_hyst"};
  ta_rwram<U_WIDTH, 2048, 4, 1> u_ram12{"t12_u"};
  ...
  hcm::zone zone13;
  ta_rwram<PRED_BITS, 2048, 4, 1> pred_ram13{"t13_pred"};
  ta_rwram<HYST_WIDTH, 1024, 4, 1> hyst_ram13{"t13_hyst"};
  ta_rwram<U_WIDTH, 2048, 4, 1> u_ram13{"t13_u"};

  // ---- Meta (update-only, 2 banks) ----
  hcm::zone meta_zone;
  ta_rwram<META_WIDTH, META_CAPACITY, 4, 1> meta_ctr0{"meta0"};
  ta_rwram<META_WIDTH, META_CAPACITY, 4, 1> meta_ctr1{"meta1"};
...
#undef HC_DISPATCH
  ...
  val<1> predict1([[maybe_unused]] val<64> inst_pc) {
    ...
    }
    {
      ...
      meta_idx.fanout(
          hard<2 + META_BANKS>{}); // 2 RAM reads + 2 idx_pipe writes
    ...
    // Per-bank meta_use_alt >= 0 means "trust alt over provider when provider
    // is weak"
    ...
    for (u64 i = 0; i < NT; i++) {
      ...
      prefetch_group_id[i].fanout(
          hard<1 + NUM_GROUPS>{}); // shift + 7 group== checks
      prefetch_pred[i].fanout(
          hard<1 + NUM_GROUPS>{}); // shift + 7 resolve chains
    }
    ...
    meta_use_alt.fanout(
        hard<4>{}); // max 4 reads per bank (groups 0-3 for bank 0)
    ...
    auto resolve_chain = [&](arr<val<1>, NT> fh, val<PRED_BITS> fb_g,
      ...
      for (u64 i = 0; i < NT + 1; i++)
        table_preds[i].fanout(hard<2>{});
    ...
    static_loop<NUM_GROUPS>([&]<u64 G>() {
      ...
      arr<val<1>, NT> group_hits = [&](u64 i) -> val<1> {
        val<1> group_ok = val<GROUP_BITS>{prefetch_group_id[i]} == hard<G>{};
    ...
    static_loop<NT>([&]<u64 I>() {
      ...
      prefetch_tag_hit[I] = val<PER_TABLE_HTAG[I]>{stored_tag} ==
                            val<PER_TABLE_HTAG[I]>{computed_tag};
      prefetch_group_id[I] = val<GROUP_BITS>{stored_tag >> PER_TABLE_HTAG[I]};
  ...
  void update_cycle([[maybe_unused]] instruction_info &block_end_info) {
    ...
    arr<val<PRED_BITS>, NUM_GROUPS> old_provider_pred =
        [&](u64 g) -> val<PRED_BITS> {
    ...
    branch_dir.fanout(hard<5>{}); // actual_dir_concat + per_group_wrong +
                                  // entry_actual + true_block + last_condbr
    ...
    static_loop<NT>([&]<u64 I>() { t_m1[I].fanout(hard<4>{}); });
    ...
    if (num_branch == 0) {
      ...
      static_loop<NT>([&]<u64 I>() {
        auto &fi = fold_idx_at<I>();
        auto &ft = fold_tag_at<I>();
        fi.apply_update(fi.compute_update(gh, hard<HIST_LEN[I]>{}, path_bits));
        ft.apply_update(ft.compute_update(gh, hard<HIST_LEN[I]>{}, path_bits));
      });
      gh.update(path_bits);
      last_condbr_dir = 0;
      true_block = 1;
    ...
    // extra_cycle(1) + alloc_trigger(1) + fb_gate(1) + acc_ctr(1) +
    // true_block(1) + bh_read(1) + bh_gate(1)
    mispredict.fanout(hard<64>{});

    // Read fb bimodal hyst BEFORE extra_cycle (cycle 1); write happens AFTER
    // (cycle 2)
    ...
    postmask.fanout(
        hard<2>{}); // candallocmask + uclearmask (U_CLEAR=DECREMENT)
    ...
    val<GROUP_BITS> alloc_gid_sib = val<GROUP_BITS>{u64(train_group)};
    ...
    // Probabilistically skips the closest candidate when alloc pressure is
    // high. alloc_rng: single read, use fo1()
    val<NT> collamask = AllocPressureSkipTarget<1>::apply<NT>(
    ...
    train_fb.fanout(
        hard<3>{}); // fb_wrong(1) + fb_write_data(1) + fb_changed(1)
    ...
    val<FB_PRED_BITS> fb_write_data =
        val<FB_PRED_BITS>{train_fb} ^ fb_flip.fo1();
    ...
    // bank_wrong_dir = OR_g(pw_g & ad_g & wrong_g) — direction from gated
    // groups Bank 0: groups 0-3
    {
    ...
    // Bank 1: groups 4-6
    {
      ...
      val<1> meta_gate1 =
          do_train & bank1_gate.fo1() & (new_meta1 != old_meta1);
      meta_gate1.fanout(hard<11>{});
      execute_if(meta_gate1, [&]() {
        meta_ctr1.write(
            val<META_IDX_BITS>{meta_idx_pipe[1][META_PIPE - 1].fo1()},
            new_meta1, mispredict);
      });
    }
    ...
    static_loop<NT>([&]<u64 I>() {
      ...
      // For provider table: stored group == train_group (guaranteed by
      // resolution). For alloc target: new entry is for train_group.
      val<1> table_wrong =
      ...
      execute_if(gate_pred, [&]() {
        pred_ram_at<I>().write(val<IDX_BITS[I]>{train_idx[I]}, entry_actual_dir,
                               mispredict);
      });
      ...
      execute_if(gate_alloc, [&]() {
        ...
        auto full_tag = concat(alloc_gid.fo1(),
                               val<PER_TABLE_HTAG[I]>{train_ctag[I].fo1()});
      ...
      // u_inc(2) + u_dec(2) + decay_select(2) + merged_select(1) +
      // merged_changed(1)
      ...
      auto u_dec = val<U_WIDTH>{old_u - val<U_WIDTH>{old_u != hard<0>{}}};
      ...
      val<U_WIDTH> base_newu = [&]() -> val<U_WIDTH> {
        ...
        auto ucl_mask =
            val<U_WIDTH>{uclear[I].fo1().replicate(hard<U_WIDTH>{}).concat()};
        return inc_term.fo1() | val<U_WIDTH>{ucl_mask.fo1() & u_dec.fo1()};
      }();
      ...
      execute_if(gate_u, [&]() {
        u_ram_at<I>().write(val<IDX_BITS[I]>{train_idx[I]}, merged, mispredict);
      });
    });
    ...
    auto new_alloc =
        ta_update_ctr(val<ALLOC_WIDTH>{alloc_ctr}, ~any_alloc.fo1());
    acc_ctr = new_acc.fo1();
    ...
    // NT fold_idx + NT fold_tag apply_update muxes + gh.update +
    // last_condbr_dir

...
struct tutorial_04 : predictor {
    ...
    val<6> fancy_index(val<64> pc) {
        auto combine = []  (val<6> a, val<6> b) {
            // XOR is boring, let's have some fun
            arr<val<6>, 2> c = {a,  b};
            auto d = c.make_array(val<4>{});
            return (d[0] * d[1]) / hard<17>{} + d[2];
        };
        return fold(pc.make_array(val<6>{}), combine);
    }
    ...
    val<1> predict1([[maybe_unused]] val<64> inst_pc)
    {
        ...
        // XOR.
        val<6> index = fancy_index(inst_pc);

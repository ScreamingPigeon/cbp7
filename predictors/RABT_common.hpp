...
// Graded array: linear interpolation from v_first (index 0) to v_last (index
// N-1). Useful for per-table LFSR widths where T0 (longest history) differs
// from T(N-1).
template <typename T, std::size_t N>
...
// Requires access to HIST_LEN at TATableConfig level — use via generate
// overload.
...
template <u64 THRESH> struct FixedDecayThresh {
  template <u64 I, u64 LW, u64 AW, u64 PW>
  static auto compute(val<AW> /*acc*/, val<PW> /*alloc*/) {
    constexpr u64 clamped =
        THRESH < (u64(1) << LW) ? THRESH : (u64(1) << LW) - 1;
...
template <u64 THRESH_LO, u64 THRESH_HI, u64 NT> struct GradedDecayThresh {
  template <u64 I, u64 LW, u64 AW, u64 PW>
  static auto compute(val<AW> /*acc*/, val<PW> /*alloc*/) {
    constexpr u64 t =
        NT > 1 ? THRESH_LO + (THRESH_HI - THRESH_LO) * I / (NT - 1) : THRESH_LO;
...
template <u64 FIXED> struct AccGatedDecayThresh {
  template <u64 I, u64 LW, u64 AW, u64 PW>
...
// Addresses workloads with low table contention (e.g. namd) where decay is
// harmful.
...
template <u64 THRESH_LO, u64 THRESH_HI, u64 NT> struct PressScaledDecayThresh {
  template <u64 I, u64 LW, u64 AW, u64 PW>
  static auto compute(val<AW> /*acc*/, val<PW> alloc_ctr) {
    constexpr u64 t =
        NT > 1 ? THRESH_LO + (THRESH_HI - THRESH_LO) * I / (NT - 1) : THRESH_LO;
...
template <u64 F> struct SecTagFloor {
  static constexpr bool RUNTIME = false;
...
template <u64 GATE> struct SecTagPressGated {
...
template <u64 F, u64 GATE> struct SecTagPressGatedFloor {
  static constexpr bool RUNTIME = true;
  template <u64 I, u64 NT> static constexpr bool apply() { return I >= F; }
  template <u64 I, u64 NT, u64 AW, u64 PW, u64 BW>
  static val<1> gate(val<AW> /*acc*/, val<PW> alloc_ctr, val<BW> /*benefit*/) {
...
template <u64 GATE> struct SecTagAccGated {
  static constexpr bool RUNTIME = true;
  template <u64 I, u64 NT> static constexpr bool apply() { return true; }
  template <u64 I, u64 NT, u64 AW, u64 PW, u64 BW>
...
template <u64 WIDTH, u64 THRESH> struct SecTagAdaptive {
  static constexpr bool RUNTIME = true;
...
struct Xor3SecTagHash3 {
  template <u64 SEC_TAG_BITS> static val<SEC_TAG_BITS> apply(val<64> pc) {
    static_assert(SEC_TAG_BITS == 3,
                  "Xor3SecTagHash3 is tuned for 3-bit sec_tag");
    pc.fanout(hard<3>{});
...
struct Xor3SecTagHash {
  template <u64 SEC_TAG_BITS> static val<SEC_TAG_BITS> apply(val<64> pc) {
    static_assert(SEC_TAG_BITS == 4,
                  "Xor3SecTagHash is tuned for 4-bit sec_tag");
    pc.fanout(hard<3>{});
...
struct Xor3SecTagHash5 {
  template <u64 SEC_TAG_BITS> static val<SEC_TAG_BITS> apply(val<64> pc) {
    static_assert(SEC_TAG_BITS == 5,
                  "Xor3SecTagHash5 is tuned for 5-bit sec_tag");
    pc.fanout(hard<3>{});
...
template <u64 N, u64 M, u64 B, u64 BANK_SHIFT = 0> struct ta_rwram {
  ...
  u64 stat_read_same_prev =
      0; // read bank == previous read bank (consecutive correlation)
  // XOR histogram: how many bits of the address change between consecutive
  // reads idx_xor_hist[i] = count of cycles where popcount(addr XOR prev_addr)
  // == i
  ...
  val<N> read(val<A> addr) {
...
#ifdef TAGE_MONITOR
    {
      ...
      if (stat_reads > 1) {
        ...
        u64 bank_bits_prev =
            (prev_read_addr_ >> BANK_SHIFT) & ((1u << BANK_BITS) - 1);
    ...
    stat_writes++;
    {
      ...
      if (nc_val == 0) {
        ...
        if constexpr (BANK_SHIFT == 0) {
          ...
          new_bank =
              1u << (((addr_val >> BANK_SHIFT) & ((1u << BANK_BITS) - 1)));
        }
  ...
  void print_conflict_stats(std::ostream &os = std::cerr) const {
    ...
    os << "rwram[" << name_ << "] writes=" << stat_writes
       ...
       << " lost=" << stat_lost << " ("
       << (stat_buffered > 0 ? 100.0 * stat_lost / stat_buffered : 0.0)
       << "% of buffered)"
       << " flushed=" << stat_flushed
       << " read_same_prev=" << stat_read_same_prev << " ("
       << (stat_reads > 1 ? 100.0 * stat_read_same_prev / (stat_reads - 1)
                          : 0.0)
       << "%)"
       << "\n";
...
struct TATable {
  ...
  ta_rwram<PRED_BITS, TABLE_SIZE, RWRAM_BANKS, RWRAM_BANK_SHIFT> pred_ram{
      "ta_pred"};
  ...
  ta_rwram<std::max(u64(1), HYST_WIDTH), HYST_SIZE, RWRAM_BANKS,
           RWRAM_BANK_SHIFT>
      hyst_ram{"ta_hyst"};

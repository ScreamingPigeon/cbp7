...
//
// Instead of indexing block B1's prediction with the address of block B1, we
// index it with the address of the previous block, B0, and the path followed
// out of block B0. N is the maximum number of branches predicted per cycle (at
// most one taken branch). Unless block B0 ends on an indirect jump, there are
// at most N+1 paths out of B0. The N+1 possible block predictions are read
// simultaneously from multiple banks. Once the path is known in the next cycle,
// it is used to select one of the N+1 block predictions. As the N+1 paths out
// of B0 are not equally likely, and in order to use storage evenly, the bank
// associated with a given path depends on some bits XB of B0's address. Each of
// the N predictions for B1 is associated with a lane. To use lanes evenly, the
// lane depends on B1's address.

template <u64 LOGG = 19, u64 GHIST = 16, u64 N = 7>
struct gshareN_ahead_best : predictor {
  ...
  reg<LANES>
      unordered_pred; // read prediction bits for the current block, unordered
  arr<reg<1>, LANES>
      pred; // read prediction bits for the current block, ordered

  // a true block is a block whose length is the same whether or not there is a
  // mispredict
  reg<1> true_block = 1;
  ...
  reg<N + 1> rank; // one-hot bit vector telling the rank of the current branch
                   // in the block
  ...
  ram<val<1>, (BANKS << index_bits)>
      ctr_lo[LANES]; // hysteresis bit (1=weak, 0=strong)
  ...
  void update_global_history(val<GHIST> injected_bits) {
    // optimal global history length = tradeoff between footprint and branch
    // correlations footprint is a function of global history length in branches
    // (not in blocks)
    arr<val<1>, N + 1> num_cbr = (rank << num_branch).make_array(val<1>{});
    val<GHIST> shifted_ghist =
        arr<val<GHIST>, N + 1>{[&](int i) {
          return (global_history << std::max(i, 1)) &
                 num_cbr[i].fo1().replicate(hard<GHIST>{}).concat();
        }}.fold_or();
  ...
  val<1> predict1([[maybe_unused]] val<64> inst_pc) {
    ...
    // if the previous block was not a true block, we continue using the
    // previous block predictions (golden rule: never make a predictor's inputs
    ...
    execute_if(true_block, [&]() {
      ...
      if constexpr (GHIST <= index_bits) {
        ...
        index[0] = pc_bits.fo1() ^
                   (val<index_bits>{global_history} << (index_bits - GHIST));
      } else {
  ...
  void update_cycle([[maybe_unused]] instruction_info &block_end_info) {
    ...
    arr<val<1>, LANES> access =
        arr<val<LANES>, LANES>{[&](u64 i) {
          return XL.rotate_left(i) & val<LANES>{-(i < num_branch)};
        }}.fold_or()
            .make_array(val<1>{});

    // misp bank = bit vector pointing to the lane accessed by the mispredicted
    // branch (all zero if no mispredict)
    val<LANES> misp_bank = XL.rotate_left(num_branch - 1) &
    ...
    // weak[i] = 1 iff bank #i corresponds to mispredicted branch and hysteresis
    // is weak
    ...
    execute_if(mispredict, [&]() {
      ...
      val<LANES> block_bundle =
          arr<val<1>, LANES>{[&](u64 i) {

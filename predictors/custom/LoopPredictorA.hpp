...
struct LoopPredictorA {
  ...
  LookupResult lookup(val<64> &inst_pc, [[maybe_unused]] val<1> &tage_pred, [[maybe_unused]] val<1> &tage_low_conf) {
  ...
  void train_compute(arr<val<1>, FETCH_WIDTH> &branch_taken,
    ...
    for (u64 w = 0; w < LWAYS; w++) {
      ...
      if (is_match_way) {
        ...
        staged_entries[w] = updated;
        staged_write_mask[w] = val<1>{1};
        ...
        if (try_alloc) {
          ...
          staged_entries[w] = alloc_entry;
          staged_write_mask[w] = do_alloc;
  ...
  void train_write() {
    ...
    for (u64 w = 0; w < LWAYS; w++) {
      val<1> write_mask = staged_write_mask[w];
      execute_if(write_mask, [&]() {
        lram[w].write(val<SET_BITS>{lp_idx}, val<ENTRY_BITS>{staged_entries[w]}, val<1>{0});
      });
    }
  ...
  // Staged write data for train_write
  arr<reg<ENTRY_BITS>, LWAYS> staged_entries;
  arr<reg<1>, LWAYS> staged_write_mask;

  // Control variables for two-phase training
